...
const fragmentShader = `
  ...
  void main() {
    ...
    color = mix(color, vec3(1.0), pow(abs(noise), 2.0) * intensity * 0.2); // Reduced brightness of noise
...
export function ShaderPlane({
  ...
  color1 = "#2f6b65",
  color2 = "#040807",
  scale = 10, // Added scale so it can cover the screen
}: {
...
export function EnergyRing({
  ...
  return (
    <mesh ref={mesh} position={position}>
      ...
      <meshBasicMaterial color="#2f6b65" transparent opacity={0.6} side={THREE.DoubleSide} />
    </mesh>
  )
}
